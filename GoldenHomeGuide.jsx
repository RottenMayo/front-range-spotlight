...
// ---- Card preview — Canva embed if URL is set, otherwise slot grid ----
    ...
    return (
      <div style={{ background: "rgba(194,138,44,.12)", border: `1.5px solid ${ACCENT}`, borderRadius: 14, padding: "32px 28px", textAlign: "center" }}>
        ...
        <p style={{ opacity: 0.85, margin: 0, fontSize: 15.5 }}>
          Thanks, {form.name.split(" ")[0]}. I'll reach out within 24 hours to confirm your category and walk you through the next steps.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <input className="ghg-input" placeholder="Your name *" value={form.name} onChange={set("name")} />
        <input className="ghg-input" placeholder="Business name *" value={form.business} onChange={set("business")} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <input className="ghg-input" placeholder="Email *" value={form.email} onChange={set("email")} />
        <input className="ghg-input" placeholder="Phone" value={form.phone} onChange={set("phone")} />
      </div>
      <input className="ghg-input" placeholder={`What type of service do you offer? (${availableCount} slot${availableCount !== 1 ? "s" : ""} still open)`} value={form.category} onChange={set("category")} />
      <textarea className="ghg-input" placeholder="Anything else? (optional)" rows={3} value={form.message} onChange={set("message")} style={{ resize: "vertical" }} />
      <button
        className="ghg-btn"
        onClick={submit}
        style={{ background: ACCENT, color: INK, padding: "15px", borderRadius: 10, fontSize: 16.5, marginTop: 4 }}
      >
        Reserve my spot →
      </button>
